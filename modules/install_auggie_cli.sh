...
install_auggie_cli() {
    ...
    echo -e "\n${BLUE}╔═══════════════════════════════════════════════╗${NC}"
    echo -e "${YELLOW}[BİLGİ]${NC} Auggie CLI kurulumu başlatılıyor..."
    echo -e "${BLUE}╚═══════════════════════════════════════════════╝${NC}"
    ...
    echo -e "${YELLOW}[BİLGİ]${NC} Auggie CLI npm paketinin kurulumu başlatılıyor..."
    if ! npm_install_global_with_fallback "$package_spec" "Auggie CLI"; then
        echo -e "${RED}[HATA]${NC} Auggie CLI kurulumu başarısız oldu. Paket: ${package_spec}"
        return 1
    fi
    ...
    if ! command -v auggie >/dev/null 2>&1; then
        echo -e "${RED}[HATA]${NC} 'auggie' komutu bulunamadı. PATH ayarlarınızı kontrol edin."
        return 1
    fi

    echo -e "${GREEN}[BAŞARILI]${NC} Auggie CLI sürümü: $(auggie --version 2>/dev/null || echo 'sürüm bilgisi alınamadı')"

    cat <<EOF
${CYAN}[BİLGİ]${NC} Auggie CLI, depodaki kodu anlayıp güvenli değişiklikler yapabilmeniz için şu özellikleri sunar:
...
EOF
    ...
    if [ "$interactive_mode" = true ]; then
        if [ -r /dev/tty ] && [ -w /dev/tty ]; then
            echo -e "\n${YELLOW}[BİLGİ]${NC} Augment hesabınızla giriş yapmak için '${GREEN}auggie login${NC}' komutunu çalıştırıyoruz..."
            if ! auggie login </dev/tty >/dev/tty 2>&1; then
                echo -e "${YELLOW}[UYARI]${NC} 'auggie login' komutu başarısız oldu. Gerekirse manuel olarak tekrar çalıştırın."
            fi
        else
            echo -e "\n${YELLOW}[UYARI]${NC} TTY erişimi yok; lütfen '${GREEN}auggie login${NC}' komutunu manuel olarak çalıştırın."
        fi
        read -r -p "Devam etmek için Enter'a basın..." </dev/tty || true
    else
        echo -e "\n${YELLOW}[BİLGİ]${NC} Toplu modda kimlik doğrulama atlandı. Kurulum sonrası '${GREEN}auggie login${NC}' komutunu çalıştırmayı unutmayın."
    fi

    echo -e "${GREEN}[BAŞARILI]${NC} Auggie CLI kurulumu tamamlandı! Detaylı rehber: ${AUGGIE_DOC_URL}"
}

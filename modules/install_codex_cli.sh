...
install_codex_cli() {
    ...
    echo -e "\n${BLUE}╔═══════════════════════════════════════════════╗${NC}"
    echo -e "${YELLOW}${INFO_TAG}${NC} OpenAI Codex CLI kurulumu başlatılıyor..."
    echo -e "${BLUE}╚═══════════════════════════════════════════════╝${NC}"
    
    npm install -g @openai/codex
    
    echo -e "${GREEN}${SUCCESS_TAG}${NC} Codex CLI sürümü: $(codex --version)"
    ...
    if [ "$interactive_mode" = true ]; then
        ...
        echo -e "${YELLOW}${INFO_TAG}${NC} Hangi yöntemi kullanmak istersiniz?"
        ...
        case $auth_choice in
            1)
                echo -e "\n${YELLOW}${INFO_TAG}${NC} Codex başlatılıyor, ChatGPT ile giriş yapın..."
                echo -e "${YELLOW}${INFO_TAG}${NC} Tarayıcıda açılan sayfadan giriş yapın."
                echo -e "${YELLOW}${INFO_TAG}${NC} Giriş tamamlandıktan sonra buraya dönün.\n"
                codex --auth-only 2>/dev/null || codex
                ;;
            2)
                echo -e "\n${YELLOW}${INFO_TAG}${NC} OpenAI API Key girişi"
                    ...
                    for rc_file in "$HOME/.bashrc" "$HOME/.zshrc" "$HOME/.profile"; do
                        if [ -f "$rc_file" ]; then
                            if ! grep -q 'OPENAI_API_KEY' "$rc_file"; then
                                ...
                                echo -e "${GREEN}${SUCCESS_TAG}${NC} API Key $rc_file dosyasına eklendi"
                            fi
                        fi
                    done
                    
                    export OPENAI_API_KEY="$OPENAI_KEY"
                    echo -e "${GREEN}${SUCCESS_TAG}${NC} API Key ayarlandı"
                else
                    echo -e "${RED}${ERROR_TAG}${NC} API Key boş olamaz!"
                fi
                ;;
            3)
                echo -e "${YELLOW}${INFO_TAG}${NC} Kimlik doğrulama atlandı. Daha sonra yapabilirsiniz."
                ;;
            *)
        ...
        echo -e "${YELLOW}${INFO_TAG}${NC} Lütfen daha sonra manuel olarak '${GREEN}codex${NC}' komutunu çalıştırarak kimlik doğrulama yapın."
    fi
    
    echo -e "\n${BLUE}╔═══════════════════════════════════════════════╗${NC}"
    echo -e "${YELLOW}${INFO_TAG}${NC} Codex CLI Kullanım İpuçları:"
    ...
    echo -e "\n${GREEN}${SUCCESS_TAG}${NC} OpenAI Codex CLI kurulumu tamamlandı!"
}

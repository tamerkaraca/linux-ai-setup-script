...
declare -A CLEANUP_QWEN_TEXT_EN=(
    ["cleanup_title"]="Cleaning up SuperQwen MCP Server Configuration..."
...
declare -A CLEANUP_QWEN_TEXT_TR=(
    ["cleanup_title"]="SuperQwen MCP Sunucu Yapılandırması Temizleme..."
...
cleanup_qwen_text() {
    local key="$1"
    local default_value="${CLEANUP_QWEN_TEXT_EN[$key]:-$key}"
    if [ "${LANGUAGE:-en}" = "tr" ]; then
        printf "%s" "${CLEANUP_QWEN_TEXT_TR[$key]:-$default_value}"
    else
...
cleanup_qwen_mcp() {
    echo -e "\n${BLUE}╔═══════════════════════════════════════════════╗${NC}"
    echo -e "${YELLOW}${INFO_TAG}${NC} $(cleanup_qwen_text cleanup_title)"
    echo -e "${BLUE}╚═══════════════════════════════════════════════╝${NC}"
    
    SETTINGS_FILE="$HOME/.qwen/settings.json"
    
    if ! command -v jq &> /dev/null; then
        echo -e "${RED}${ERROR_TAG}${NC} $(cleanup_qwen_text jq_not_found)"
        echo -e "${YELLOW}${INFO_TAG}${NC} $(cleanup_qwen_text jq_install_prompt)"
        return 1
    fi
    ...
    if ! jq -e '.mcpServers' "$SETTINGS_FILE" > /dev/null; then
        echo -e "${YELLOW}${INFO_TAG}${NC} $(cleanup_qwen_text mcp_settings_missing)"
        return 0
    fi
    ...
    if [ ${#mcp_servers[@]} -eq 0 ]; then
        echo -e "${YELLOW}${INFO_TAG}${NC} $(cleanup_qwen_text no_mcp_servers)"
        return 0
    fi

    echo -e "\n${YELLOW}$(cleanup_qwen_text select_servers_prompt)${NC}"
    local index=1
    for server in "${mcp_servers[@]}"; do
        echo -e "  ${GREEN}${index}${NC} - ${server}"
        index=$((index + 1))
    done
    echo -e "  ${RED}0${NC} - $(cleanup_qwen_text cancel_option)"
    echo -e "${YELLOW}${INFO_TAG}${NC} $(cleanup_qwen_text multiple_selection_hint)"

    read -r -p "$(cleanup_qwen_text choice_prompt)" choices </dev/tty
    if [ "$choices" = "0" ] || [ -z "$choices" ]; then
        echo -e "${YELLOW}${INFO_TAG}${NC} $(cleanup_qwen_text operation_canceled)"
        return 0
    fi
    ...
    if [ "$changes_made" = true ]; then
        ...
        rm -f "$temp_file"
        echo -e "${YELLOW}${INFO_TAG}${NC} $(cleanup_qwen_text no_changes_made)"
    fi
}

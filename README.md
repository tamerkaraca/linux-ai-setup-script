...
- **GLM-4.6 (z.ai)** – https://z.ai/model-api üzerinden alınan API key’i maskeleyerek `~/.claude/settings.json` dosyasına yazar. Menü, varsayılan `https://api.z.ai/api/anthropic` adresini tek tuşla seçmenize veya özel bir base URL girmenize izin verir; `ANTHROPIC_DEFAULT_*` modelleri GLM-4.6/GLM-4.5-Air olarak belirlenir.
- **Moonshot kimi-k2** – https://platform.moonshot.ai/docs/guide/agent-support#install-claude-code rehberindeki adımlara göre önce Node.js ≥ 18 koşulunu doğrular (gerekirse Claude Code CLI’yi yeniden kurmayı teklif eder), ardından Moonshot API key’inizi ve taban URL’yi (varsayılan `https://api.moonshot.ai/anthropic` veya önceki ayarınız) seçtirir; sonrasında tercih edilen modeli (`kimi-k2-0711-preview` veya `kimi-k2-turbo-preview`) kaydeder. Tüm değerler `~/.claude/settings.json` dosyasına yazıldığı için daha sonra sadece Enter’a basarak anahtarları koruyabilirsiniz.

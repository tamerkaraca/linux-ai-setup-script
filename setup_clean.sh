...
run_module() {
    ...
    if [ -f "$local_path" ]; then
        ...
        echo -e "${INFO_TAG} Module URL: $module_url"
...
EOF
        ...
        if [ ! -f "$remote_dir/utils/utils.bash" ]; then
            echo -e "${INFO_TAG} Downloading utils.bash to $remote_dir/utils/utils.bash..."
            echo -e "${INFO_TAG} From URL: ${SCRIPT_BASE_URL}/modules/utils/utils.bash"
            ...
            echo -e "${SUCCESS_TAG} utils.bash downloaded successfully"
        fi

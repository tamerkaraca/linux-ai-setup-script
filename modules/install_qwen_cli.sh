...
ensure_npm_available() {
    ...
    echo -e "${RED}[HATA]${NC} npm komutu bulunamadı. Lütfen 'Ana Menü -> 3' ile Node.js kurulumunu tamamlayın."
    return 1
}
...
install_qwen_cli() {
    ...
    echo -e "${YELLOW}[BİLGİ]${NC} Qwen CLI npm paketinin kurulumu başlatılıyor..."
    if ! npm_install_global_with_fallback "$package_spec" "Qwen CLI"; then
        echo -e "${RED}[HATA]${NC} Qwen CLI kurulumu başarısız oldu. Paket: ${package_spec}"
        return 1
    fi
    ...
    if ! command -v qwen >/dev/null 2>&1; then
        echo -e "${RED}[HATA]${NC} 'qwen' komutu bulunamadı. PATH ayarlarınızı kontrol edin."
        return 1
    fi

    echo -e "${GREEN}[BAŞARILI]${NC} Qwen CLI sürümü: $(qwen --version 2>/dev/null)"

    if [ "$interactive_mode" = true ]; then
        echo -e "\n${YELLOW}[BİLGİ]${NC} Şimdi Qwen CLI ile kimlik doğrulaması yapmalısınız."
        echo -e "${CYAN}  qwen login${NC} komutunu çalıştırarak hesabınızla giriş yapın."
        if [ -r /dev/tty ] && [ -w /dev/tty ]; then
            qwen login </dev/tty >/dev/tty 2>&1 || echo -e "${YELLOW}[UYARI]${NC} Oturum açma sırasında bir hata oluştu. Gerekirse manuel olarak 'qwen login' çalıştırın."
        else
            echo -e "${YELLOW}[UYARI]${NC} TTY erişimi yok. Lütfen manuel olarak 'qwen login' çalıştırın."
        fi
        read -r -p "Devam etmek için Enter'a basın..." </dev/tty || true
    else
        echo -e "\n${YELLOW}[BİLGİ]${NC} Toplu kuruluma göre kimlik doğrulama adımı atlandı."
        echo -e "${YELLOW}[BİLGİ]${NC} Kurulum sonrası '${GREEN}qwen login${NC}' komutunu manuel olarak çalıştırmayı unutmayın."
    fi

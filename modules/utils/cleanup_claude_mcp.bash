...
cleanup_claude_mcp() {
    ...
    if [ ! -f "$SETTINGS_FILE" ]; then
        local msg_format
        msg_format="$(cleanup_claude_text settings_not_found)"
    ...
    for choice in "${SELECTED_INDICES[@]}"; do
        ...
        if ! [[ "$choice" =~ ^[1-9][0-9]*$ ]]; then
            local msg_format
            msg_format="$(cleanup_claude_text invalid_selection)"
        ...
        if [ $idx -lt 0 ] || [ $idx -ge ${#mcp_servers[@]} ]; then
            local msg_format
            msg_format="$(cleanup_claude_text unsupported_selection)"
            local msg="${msg_format//\%s/$choice}"
            echo -e "${RED}${ERROR_TAG}${NC} $msg"
            continue
        fi
        ...
        local msg_format
        msg_format="$(cleanup_claude_text removing_server)"
        ...
        if jq "del(.mcpServers.\"$server_to_remove\")" "$temp_file" > "$temp_file.tmp" && mv "$temp_file.tmp" "$temp_file"; then
            local msg_format
            msg_format="$(cleanup_claude_text server_removed)"
            local msg="${msg_format//\%s/$server_to_remove}"
            echo -e "${GREEN}${SUCCESS_TAG}${NC} $msg"
            changes_made=true
        else
            local msg_format
            msg_format="$(cleanup_claude_text server_remove_error)"
            local msg="${msg_format//\%s/$server_to_remove}"
            echo -e "${RED}${ERROR_TAG}${NC} $msg"
        fi
    done
    ...
    if [ "$changes_made" = true ]; then
        ...
        local msg_format
        msg_format="$(cleanup_claude_text changes_saved)"
        local msg="${msg_format//\%s/$SETTINGS_FILE}"
        echo -e "${GREEN}${SUCCESS_TAG}${NC} $msg"
    else
        rm -f "$temp_file"

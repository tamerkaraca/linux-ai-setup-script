...
configure_zsh() {
    log_info_detail "$(configure_zsh_text config_title)"
    
    # Check if zsh is installed
    if ! command -v zsh >/dev/null 2>&1; then
        log_error_detail "$(configure_zsh_text zsh_missing)"
        return 1
    fi
    ...
    if [ -f "$zshrc_file" ] && [ ! -f "$backup_file" ]; then
        ...
        log_info_detail "$(configure_zsh_text backup_info)"
    fi
    ...
    log_info_detail "Creating optimized ~/.zshrc configuration..."
...
EOF
    ...
    log_success_detail "$(configure_zsh_text config_success)"
    
    if [ ${#available_plugins[@]} -gt 0 ]; then
        log_info_detail "$(configure_zsh_text plugins_info): ${available_plugins[*]}"
    fi
    
    if [ ${#available_themes[@]} -gt 0 ]; then
        log_info_detail "$(configure_zsh_text theme_info): ${available_themes[*]}"
    fi
    
    log_info_detail "$(configure_zsh_text reload_info)"
    log_success_detail "$(configure_zsh_text config_done)"
}

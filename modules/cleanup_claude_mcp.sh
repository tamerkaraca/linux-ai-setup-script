...
declare -A CLEANUP_CLAUDE_TEXT_EN=(
    ["cleanup_title"]="Cleaning up SuperClaude MCP Server Configuration..."
...
declare -A CLEANUP_CLAUDE_TEXT_TR=(
    ["cleanup_title"]="SuperClaude MCP Sunucu Yapılandırması Temizleme..."
...
cleanup_claude_text() {
    local key="$1"
    local default_value="${CLEANUP_CLAUDE_TEXT_EN[$key]:-$key}"
    if [ "${LANGUAGE:-en}" = "tr" ]; then
        printf "%s" "${CLEANUP_CLAUDE_TEXT_TR[$key]:-$default_value}"
    else
...
cleanup_claude_mcp() {
    echo -e "\n${BLUE}╔═══════════════════════════════════════════════╗${NC}"
    echo -e "${YELLOW}${INFO_TAG}${NC} $(cleanup_claude_text cleanup_title)"
    echo -e "${BLUE}╚═══════════════════════════════════════════════╝${NC}"
    
    SETTINGS_FILE="$HOME/.claude/settings.json"
    
    if ! command -v jq &> /dev/null; then
        echo -e "${RED}${ERROR_TAG}${NC} $(cleanup_claude_text jq_not_found)"
        echo -e "${YELLOW}${INFO_TAG}${NC} $(cleanup_claude_text jq_install_prompt)"
        return 1
    fi
    
    if [ ! -f "$SETTINGS_FILE" ]; then
        printf -v msg "$(cleanup_claude_text settings_not_found)" "$SETTINGS_FILE"
        echo -e "${YELLOW}${INFO_TAG}${NC} $msg"
        return 0
    fi

    if ! jq -e '.mcpServers' "$SETTINGS_FILE" > /dev/null; then
        echo -e "${YELLOW}${INFO_TAG}${NC} $(cleanup_claude_text mcp_settings_missing)"
        return 0
    fi
    ...
    if [ ${#mcp_servers[@]} -eq 0 ]; then
        echo -e "${YELLOW}${INFO_TAG}${NC} $(cleanup_claude_text no_mcp_servers)"
        return 0
    fi

    echo -e "\n${YELLOW}$(cleanup_claude_text select_servers_prompt)${NC}"
    local index=1
    for server in "${mcp_servers[@]}"; do
        echo -e "  ${GREEN}${index}${NC} - ${server}"
        index=$((index + 1))
    done
    echo -e "  ${RED}0${NC} - $(cleanup_claude_text cancel_option)"
    echo -e "${YELLOW}${INFO_TAG}${NC} $(cleanup_claude_text multiple_selection_hint)"

    read -r -p "$(cleanup_claude_text choice_prompt)" choices </dev/tty
    if [ "$choices" = "0" ] || [ -z "$choices" ]; then
        echo -e "${YELLOW}${INFO_TAG}${NC} $(cleanup_claude_text operation_canceled)"
        return 0
    fi
    ...
    for choice in "${SELECTED_INDICES[@]}"; do
        ...
        if ! [[ "$choice" =~ ^[1-9][0-9]*$ ]]; then
            printf -v msg "$(cleanup_claude_text invalid_selection)" "$choice"
        ...
        if [ $idx -lt 0 ] || [ $idx -ge ${#mcp_servers[@]} ]; then
            printf -v msg "$(cleanup_claude_text unsupported_selection)" "$choice"
            echo -e "${RED}${ERROR_TAG}${NC} $msg"
            continue
        fi

        local server_to_remove="${mcp_servers[$idx]}"
        printf -v msg "$(cleanup_claude_text removing_server)" "$server_to_remove"
        echo -e "${YELLOW}${INFO_TAG}${NC} $msg"
        
        jq "del(.mcpServers.\"$server_to_remove\")" "$temp_file" > "$temp_file.tmp" && mv "$temp_file.tmp" "$temp_file"
        if [ $? -eq 0 ]; then
            printf -v msg "$(cleanup_claude_text server_removed)" "$server_to_remove"
            echo -e "${GREEN}${SUCCESS_TAG}${NC} $msg"
            changes_made=true
        else
            printf -v msg "$(cleanup_claude_text server_remove_error)" "$server_to_remove"
            echo -e "${RED}${ERROR_TAG}${NC} $msg"
        fi
    done
    ...
    if [ "$changes_made" = true ]; then
        ...
        printf -v msg "$(cleanup_claude_text changes_saved)" "$SETTINGS_FILE"
        echo -e "${GREEN}${SUCCESS_TAG}${NC} $msg"
    else
        rm -f "$temp_file"
        echo -e "${YELLOW}${INFO_TAG}${NC} $(cleanup_claude_text no_changes_made)"
    fi
}

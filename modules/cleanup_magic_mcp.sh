...
cleanup_magic_mcp() {
    ...
    if [ ! -f "$SETTINGS_FILE" ]; then
        printf -v msg "$(cleanup_magic_text settings_not_found)" "$SETTINGS_FILE"
    ...
    for choice in "${SELECTED_INDICES[@]}"; do
        ...
        if ! [[ "$choice" =~ ^[1-9][0-9]*$ ]]; then
            printf -v msg "$(cleanup_magic_text invalid_selection)" "$choice"
        ...
        if [ $idx -lt 0 ] || [ $idx -ge ${#mcp_servers[@]} ]; then
            printf -v msg "$(cleanup_magic_text unsupported_selection)" "$choice"
            echo -e "${RED}${ERROR_TAG}${NC} $msg"
            continue
        fi

        local server_to_remove="${mcp_servers[$idx]}"
        printf -v msg "$(cleanup_magic_text removing_server)" "$server_to_remove"
        echo -e "${YELLOW}${INFO_TAG}${NC} $msg"
        
        jq "del(.mcpServers.\"$server_to_remove\")" "$temp_file" > "$temp_file.tmp" && mv "$temp_file.tmp" "$temp_file"
        if [ $? -eq 0 ]; then
            printf -v msg "$(cleanup_magic_text server_removed)" "$server_to_remove"
            echo -e "${GREEN}${SUCCESS_TAG}${NC} $msg"
            changes_made=true
        else
            printf -v msg "$(cleanup_magic_text server_remove_error)" "$server_to_remove"
            echo -e "${RED}${ERROR_TAG}${NC} $msg"
        fi
    done
    ...
    if [ "$changes_made" = true ]; then
        ...
        printf -v msg "$(cleanup_magic_text changes_saved)" "$SETTINGS_FILE"
        echo -e "${GREEN}${SUCCESS_TAG}${NC} $msg"
    else
        rm -f "$temp_file"
